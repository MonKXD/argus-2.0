...
- Phase: 1 complete. Phase 2 (engine spike) 17/18 — blocked on T-2.18. Phase 3 (auth, persistence, intake, orchestration) 4/14 done (T-3.01, T-3.02, T-3.04, T-3.05), T-3.03 partially done (Firestore rules; Storage rules wait for T-3.06's upload paths).
- Task: none in progress
- Next up: T-3.06 (upload flow, which also unblocks T-3.03's deferred Storage rules) or T-3.07 (URL/pasted-text sources) — either can proceed; T-3.08 (run orchestrator) is the one that actually needs the engine pipeline wired to `FirestoreStore`/a real `Run` document, so it's the natural next big lift, and is what T-3.05's Sources step and disabled "Start analysis" button are both waiting on. T-2.18 (phase gate) stays blocked in parallel — see below.
...
| 3 | Auth, persistence, intake, orchestration | 14 | 4 | In progress |
