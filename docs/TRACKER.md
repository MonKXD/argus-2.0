...
Last updated: 2026-09-23 (T-3.06)
...
- Phase: 1 complete. Phase 2 (engine spike) 17/18 — blocked on T-2.18. Phase 3 (auth, persistence, intake, orchestration) 6/14 done (T-3.01, T-3.02, T-3.03, T-3.04, T-3.05, T-3.06).
- Task: none in progress
- Next up: T-3.07 (URL/pasted-text sources, small — the one remaining honest stub in the Sources step) or T-3.08 (run orchestrator, the bigger lift both T-3.05's disabled "Start analysis" button and the real INGEST-at-registration design in T-3.06 are waiting on). T-2.18 (phase gate) stays blocked in parallel — see below.
...
| 3 | Auth, persistence, intake, orchestration | 14 | 6 | In progress |
...
- [x] **T-2.04** DOCX, XLSX, CSV, TXT, MD extractors behind an `Extractor` interface · M · FR-ENG-01
- [x] **T-2.05** SSRF-safe website ingestion (limited crawl, robots.txt, readable text) · M · FR-ENG-01, NFR-03
- [x] **T-2.06** Evidence chunking, locators, hashes, sanitisation, injection-pattern detection · S · FR-ENG-01, FR-ENG-13
- [x] **T-2.07** LLM client: model roles, forced-tool structured output, repair, caching, usage and budget · M · FR-ENG-11, FR-ENG-13
- [x] **T-2.08** Fact extraction step with quote validation (V1) · M · FR-ENG-02
...
- [x] **T-2.17** CLI `pnpm analyze` with progress output and JSON report · S · FR-ENG-01, FR-ENG-05
...
- [x] **T-3.03** Firestore and Storage rules with emulator tests · M · NFR-03 — Firestore rules done at T-3.03; Storage rules (real, matching SCHEMA.md, with a real-emulator rules test suite) completed at T-3.06 once the upload paths existed. `pnpm test:rules` now covers both.
- [x] **T-3.04** Analyses API: create, list, get, update, `assertOwns()` · M · FR-INT-01, FR-INT-06, FR-DSH-01
