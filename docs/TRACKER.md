...
- Next up: T-2.10 (scoring module with the AI_SPEC 5.4 golden test — overall combination; dimension-level score/confidence landed at T-2.09)
...
| 2 | Engine spike (CLI-first) | 18 | 9 | In progress |
...
- [ ] **T-2.10** Scoring module with the AI_SPEC 5.4 golden test (now mainly the overall/cross-dimension combination — 5.3's weighted score, coverage gate, critical-flag cap; per-dimension score/confidence, 5.2, already landed at T-2.09 since `DimensionAnalysis` needs them to be schema-valid) · M · FR-ENG-06
