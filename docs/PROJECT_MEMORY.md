...
- 2026-09-22: T-1.11 done (landing page). Pulled forward `ClaimInline` and `EvidenceRailContent` (D-031, own commit) after finding DESIGN required them but the tracker scheduled the full versions in Phase 4 — asked the user, who chose to pull forward a minimal version. Also caught and fixed a real path-vs-doc mismatch: T-1.09/T-1.10 built the app shell in `src/components/shell/` without checking TRD's repo map, which names `src/components/layout/`; renamed via `git mv` before it compounded further (own commit, see gotchas). Landing page itself: `src/components/marketing/` (site header/footer, hero, four-kinds, how-it-works, report tour, compare teaser, trust section, final CTA), sections in DESIGN 5.6's exact order, hero copy verbatim, footer disclaimer verbatim (PRD section 15). The hero's "one orchestrated moment" (DESIGN section 8: sentences resolve from hatched/muted into their real status, ~1.6s, once) is pure CSS — `@keyframes` in globals.css nested inside `@media (prefers-reduced-motion: no-preference)`, staggered per item via an inline `--hero-delay` custom property, so reduced-motion visitors see the final state immediately with no JS branch needed. Verified `motion-safe:`/`motion-reduce:` and the `supports-[...]:` variant both compile as expected by grepping compiled CSS before relying on them (R-PRC-08), same discipline as T-1.10. Replaced the old placeholder smoke test with real assertions (headline, CTA, disclaimer). `pnpm check`, `pnpm build`, `pnpm test:e2e` all green; verified in a real browser at 1440/768/390 plus a reduced-motion pass, no console/hydration errors. Next: T-1.12 (`/sample` preview). Older entries: `docs/memory-archive/2026-09-session-log.md`.
