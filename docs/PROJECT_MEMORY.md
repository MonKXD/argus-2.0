...
- 2026-09-22: T-1.12 done (`/sample` preview). DemoBanner + startup header, ScoreGauge + DimensionRadar, two narrative sections (Executive summary, Investment overview) with real, selectable `ClaimInline` rows, and an `EvidenceRailContent` that updates on selection. Evidence rail is a docked 360px column at `lg+`, reflowing inline below the sections on narrow screens via plain CSS grid placement rather than DESIGN's literal bottom sheet (D-032) — a real modal sheet needs a `matchMedia` check to avoid trapping focus in a hidden dialog on desktop, not worth it for this preview; T-4.08 gets the real thing once there's a full report page. `pnpm check`, `pnpm build`, `pnpm test:e2e` all green; verified at 1440/768/390, no console errors, claim selection updates the rail correctly. Next: T-1.13 (dashboard on demo data). Older entries: `docs/memory-archive/2026-09-session-log.md`.
