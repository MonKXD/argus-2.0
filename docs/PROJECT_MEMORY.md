...
- 2026-09-22: T-1.03 (EvidenceMarker, StatusBadge, ReliabilityChip) and T-1.04 (EvidenceBar) done. Pulled `src/lib/schema/enums.ts` forward from Phase 2 (D-024) rather than inventing a UI-only status type. All four evidence textures verified distinguishable in real browser screenshots, not just DOM structure — caught and fixed two real bugs this way (see gotchas): EvidenceBar's MISSING segment was invisible against its own track, and its `role="img"` trigger wasn't keyboard-focusable so the "hover or focus" legend was unreachable by keyboard. `forced-colors:` handling is best-effort — no modern-web-guidance guide exists for it (unlike T-1.02's overlay primitives), worth a real assistive-tech pass at T-1.16. `pnpm check`, `pnpm build`, `pnpm test:e2e` all green. Next: T-1.05 (ScoreGauge).
