...
- 2026-09-22: T-1.03 (EvidenceMarker, StatusBadge, ReliabilityChip) done. Pulled `src/lib/schema/enums.ts` forward from Phase 2 (D-024) rather than inventing a UI-only status type. All four evidence textures verified distinguishable in a real browser screenshot, not just DOM structure. `forced-colors:` handling is best-effort — no modern-web-guidance guide exists for it (unlike T-1.02's overlay primitives), worth a real assistive-tech pass at T-1.16. `pnpm check`, `pnpm build`, `pnpm test:e2e` all green. Next: T-1.04 (EvidenceBar).
