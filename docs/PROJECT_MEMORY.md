...
- 2026-09-22: T-1.08 done (demo dataset, `DemoBanner`). Pulled the rest of SCHEMA.md sections 2-5 forward (D-026) — `src/lib/schema/{ids,evidence,claims,report,analysis,run}.ts` — since typing demo data as `Analysis`/`Report` needed the real shapes, not a shadow type. Fixed two now-deprecated Zod APIs the doc's snippets used (D-027: `z.iso.datetime()`, `z.url()`). Built one fully-worked fictional analysis (Loopwell: sources, evidence, facts, all four claim statuses, flags, checklist, all 8 dimension analyses) plus three lighter analyses (scored, insufficient-evidence, in-progress-with-run) for list diversity — every fixture is schema-parsed at module load and re-asserted in `tests/unit/demo-data.test.ts`, including that each fact's quote is a real substring of its evidence text (R-AI-03). Demo IDs are deterministic digit strings (D-028), not a real ULID generator (still Phase 2). `pnpm check`, `pnpm build` green; verified in a real browser (no console/hydration errors) with the demo data feeding the existing ScoreGauge/DimensionRadar. Next: T-1.09 (AppShell). Older entries: `docs/memory-archive/2026-09-session-log.md`.
