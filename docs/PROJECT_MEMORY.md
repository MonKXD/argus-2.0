...
- 2026-09-22: T-1.03 through T-1.06 done (EvidenceMarker, StatusBadge, ReliabilityChip, EvidenceBar, ScoreGauge, DimensionRadar). Pulled `src/lib/schema/enums.ts` and confidence buckets forward from Phase 2 (D-024, D-025) rather than inventing UI-only vocabulary. Real bugs found via browser screenshots/console, not just DOM-structure tests: EvidenceBar's MISSING segment invisible against its track, its legend trigger not keyboard-focusable, and a real SSR hydration mismatch in the shared chart geometry (see gotchas) affecting both ScoreGauge and DimensionRadar. `forced-colors:` handling is best-effort (no guide exists for it). `pnpm check`, `pnpm build`, `pnpm test:e2e` all green. Next: T-1.07 (Sparkline/bar chart wrappers). Older entries: `docs/memory-archive/2026-09-session-log.md`.
