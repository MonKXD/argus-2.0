...
- 2026-09-22: T-1.13 done (dashboard on demo data). `KpiStrip` (analyses/average score/in progress/watchlisted, one continuous row per DESIGN 5.2), `AnalysesTable` (the existing T-1.02 `DataTable`'s first real usage), `InProgressPanel` (new `StepProgress` component against Fernway Health's real demo run steps), `WatchlistPanel`, and `MarketIntelligencePanel` (sector mix only — see D-033 for why not the full mockup). No "Recent activity" panel and `DemoBanner` placed on the page, not `AppShell` (both D-033). Caught and fixed a real bug via direct DOM measurement, not just a screenshot: the analyses table overflowed the page horizontally at narrow widths instead of scrolling within its own `overflow-x-auto` wrapper, the classic grid/flex-item `min-width: auto` footgun (see gotchas) — `fullPage` screenshots don't reveal this since they only scroll vertically. Fixed with `min-w-0` on the table's grid-item wrapper. `pnpm check`, `pnpm build`, `pnpm test:e2e` all green; verified at 1440/768/390. Next: T-1.14 (analyses list with search and sort). Older entries: `docs/memory-archive/2026-09-session-log.md`.
